...
| `/c/$slug` | Shipped (new) | Coach website landing page — the single page every REPs trainer can use as their public website. Slim REPs chrome bar, hero, trust strip, services grid, about, venues + cities, transformations, testimonials, qualifications, FAQ, social row (outbound links only), sticky mobile enquire bar. Tokenised accent palette (`--coach-accent-*`). All CTAs deep-link to `/pro/$slug/enquire`. `noindex` until profile data is wired. Mock data for `james-wilson`. See `mem://design/coach-website`. |
