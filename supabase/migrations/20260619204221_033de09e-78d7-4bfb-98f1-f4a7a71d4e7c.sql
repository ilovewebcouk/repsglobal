-- 1. New columns on reviews for reply edit + email idempotency
ALTER TABLE public.reviews
  ADD COLUMN IF NOT EXISTS response_edited_at timestamptz,
  ADD COLUMN IF NOT EXISTS response_notified_at timestamptz;

-- 2. RPC: upsert pro reply (1..1000 chars), only owner + approved review.
...
-- 3. RPC: clear pro reply (delete). Same ownership + approval guard.
