...
BEGIN
  ...
  -- Skip the signup grant when the tier has no signup credits configured
  -- (e.g. training_provider). grant_credits() rejects amount <= 0.
  IF COALESCE(v_policy.signup_grant, 0) <= 0 THEN
    RETURN NEW;
  END IF;
