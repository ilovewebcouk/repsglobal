...
import { Camera, CheckCircle2, GraduationCap, Loader2, Upload } from "lucide-react";
import { toast } from "sonner";

import {
  lookupCertUploadSession,
  submitCertFromMobile,
} from "@/lib/cpd/cpd.functions";

export const Route = createFileRoute("/u/cpd/$sessionId")({
  head: () => ({
    meta: [
      { title: "Upload certificate — REPS" },
      { name: "robots", content: "noindex,nofollow" },
    ],
  }),
  component: MobileCertUploadPage,
});
...
function MobileCertUploadPage() {
  const { sessionId } = Route.useParams();
  const lookup = useServerFn(lookupCertUploadSession);
  const submit = useServerFn(submitCertFromMobile);

  const sessionQ = useQuery({
    queryKey: ["mobile-cert-session", sessionId],
    ...
    <div className="min-h-dvh bg-reps-ink px-5 pb-10 pt-8 text-white">
      <div className="mx-auto flex max-w-md flex-col gap-5">
        <div className="flex items-center gap-2 text-[13px] font-semibold text-white/65">
          <GraduationCap className="size-4 text-reps-orange" />
          REPS Certificate upload
        </div>
