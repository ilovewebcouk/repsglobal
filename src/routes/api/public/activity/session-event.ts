    ...
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          if (sessionErr) {
            console.error("[activity/session-event] session upsert failed", sessionErr.message);
          }
          ...
          if (error) {
            ...
            // Beacon endpoint — never surface 500s to the browser.
            return new Response(null, { status: 204 });
          }
          try {
          ...
          // Beacon endpoint — never propagate 500s to the client; log for us.
          console.error("[activity/session-event] handler failed", e);
          return new Response(null, { status: 204 });
        }
      },
