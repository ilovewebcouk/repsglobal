...
import { ACADEMY_COURSES, LEVEL_LABELS } from "@/lib/training-academy";
...
type SortKey = "recommended" | "cpd-desc" | "price-asc" | "level-asc";
...
function TrainingAcademyPage() {
  ...
  const filtered = useMemo(() => {
    ...
    const list = ACADEMY_COURSES.filter((c) => {
      ...
      if (filters.providers.length > 0 && !filters.providers.includes(c.provider.slug))
        return false;
    ...
    const sorted = [...list];
    if (sort === "cpd-desc") sorted.sort((a, b) => b.cpdPoints - a.cpdPoints);
    else if (sort === "price-asc") sorted.sort((a, b) => a.priceFromGBP - b.priceFromGBP);
    else if (sort === "level-asc")
      sorted.sort((a, b) => LEVEL_ORDER[a.level] - LEVEL_ORDER[b.level]);
    return sorted;
  }, [filters, sort]);
  ...
  return (
    <div className="min-h-screen bg-reps-ivory">
      <PublicHeader />
      <h1 className="sr-only">REPs Training Academy — endorsed qualifications and CPD</h1>

      {/* Hero — light band */}
      <section className="bg-reps-ivory">
        <div className="mx-auto max-w-[1320px] px-6 pb-10 pt-14 lg:px-10 lg:pb-14 lg:pt-20">
          <span className="inline-flex items-center gap-1.5 rounded-full border border-reps-orange/25 bg-reps-orange/10 px-3 py-1 text-[11.5px] font-bold uppercase tracking-[0.14em] text-reps-orange">
            <GraduationCap className="h-3.5 w-3.5" />
            REPs Training Academy
          </span>

          <h2 className="mt-5 max-w-[880px] font-display text-[32px] font-bold leading-[1.05] text-reps-charcoal sm:text-[42px] lg:text-[52px]">
            Every REPs-endorsed course,{" "}
            <span className="text-reps-orange">in one catalogue.</span>
          </h2>

          <p className="mt-5 max-w-[640px] text-[16px] leading-relaxed text-reps-muted-light">
            Qualifications and CPD from training providers that have applied for
            and passed the REPs endorsement review. Filter by level, specialism,
            delivery and CPD points — then head straight to the provider to enrol.
          </p>

          <ul className="mt-6 flex flex-wrap gap-x-5 gap-y-2 text-[12.5px] font-medium text-reps-muted-light">
      ...
      {/* Sticky search bar */}
      <div className="sticky top-[72px] z-30 border-y border-reps-stone bg-reps-ivory/95 backdrop-blur">
        <div className="mx-auto flex max-w-[1320px] items-center gap-3 px-6 py-3 lg:px-10">
          <InputGroup className="h-11 flex-1 rounded-[12px] border-reps-stone bg-white">
            <InputGroupAddon>
            ...
            <InputGroupInput
              ...
              className="text-[13.5px] text-reps-charcoal placeholder:text-reps-muted-light/70"
            />
          </InputGroup>
          ...
          <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
            <SheetTrigger asChild>
              <Button
                type="button"
                variant="outline"
                className="h-11 shrink-0 rounded-[10px] border-reps-stone bg-white px-4 text-[13px] font-semibold text-reps-charcoal shadow-none hover:bg-reps-warm-white lg:hidden"
              >
            ...
            <SheetContent side="left" className="w-[320px] overflow-y-auto bg-reps-ivory p-6 sm:w-[360px]">
              <SheetHeader className="p-0 pb-4">
                <SheetTitle className="text-reps-charcoal">Filters</SheetTitle>
                <SheetDescription className="text-reps-muted-light">
          ...
          <Select value={sort} onValueChange={(v) => setSort(v as SortKey)}>
            <SelectTrigger className="h-11 w-[180px] shrink-0 rounded-[10px] border-reps-stone bg-white text-[13px] font-medium text-reps-charcoal shadow-none">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectGroup>
        ...
        <div className="mx-auto max-w-[1320px] px-6 py-8 lg:px-10 lg:py-10">
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-[280px_minmax(0,1fr)] lg:gap-10">
            ...
            <aside className="hidden lg:block">
              <div className="sticky top-[148px] max-h-[calc(100vh-168px)]">
                <ScrollArea className="h-[calc(100vh-168px)] pr-4">
                  <AcademyFilterRail value={filters} onChange={setFilters} />
                </ScrollArea>
              </div>
            </aside>
            ...
            <div className="min-w-0">
              <p className="mb-4 text-[13px] text-reps-muted-light">
                Showing{" "}
              ...
              {filtered.length === 0 ? (
                <Empty className="rounded-[18px] border border-reps-stone bg-white">
                  <EmptyHeader>
                    <EmptyTitle className="text-reps-charcoal">
                      No courses match those filters
                    </EmptyTitle>
                    <EmptyDescription className="text-reps-muted-light">
...
// Suppress unused import warning — LEVEL_LABELS is re-exported for downstream usage.
void LEVEL_LABELS;
