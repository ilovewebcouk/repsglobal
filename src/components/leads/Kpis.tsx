...
export function KpiStrip({ kpis, locked }: { kpis: LeadKpis | undefined; locked: boolean }) {
  return (
    <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">
      <KpiTile
        label="Active leads"
        value={kpis ? String(kpis.active_leads) : "—"}
        icon={Inbox}
      />
      <KpiTile
        label="Hot leads"
      ...
      />
      <ProLock locked={locked} feature="reply-time">
        <KpiTile
          label="Reply time"
          value={kpis?.reply_time_avg_hours !== null && kpis?.reply_time_avg_hours !== undefined
      ...
      <ProLock locked={locked} feature="conversion">
        <KpiTile
          label="Conversion 30d"
          value={kpis?.conversion_pct_30d !== null && kpis?.conversion_pct_30d !== undefined
            ? `${Math.round(kpis.conversion_pct_30d)}%`
            : "—"}
          icon={TrendingUp}
        />
      </ProLock>
      <ProLock locked={locked} feature="pipeline value">
        <KpiTile
          label="Pipeline value"
          value={kpis ? fmtPence(kpis.pipeline_value_pence) : "—"}
          icon={PoundSterling}
        />
      </ProLock>
      <ProLock locked={locked} feature="AI forecast">
        <KpiTile
          label="AI forecast 30d"
          value={kpis ? fmtPence(kpis.predicted_revenue_30d_pence ?? 0) : "—"}
          icon={Sparkles}
        />
      </ProLock>
    </div>
...
export function BottomCards({ kpis, locked }: { kpis: LeadKpis | undefined; locked: boolean }) {
  return (
    <div className="grid grid-cols-1 gap-3 lg:grid-cols-3">
      ...
      <div className="rounded-[16px] border border-reps-border bg-reps-panel p-4">
        ...
        <p className="mt-1 text-[12px] text-white/55">
          Leads where the trainer set a follow-up time in the next 48 hours.
        </p>
      </div>
      ...
      {/* Conversion funnel */}
      <ProLock locked={locked} feature="conversion funnel">
        <div className="rounded-[16px] border border-reps-border bg-reps-panel p-4">
