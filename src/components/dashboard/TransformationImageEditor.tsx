...
const TARGET_W = 1600;
const TARGET_H = 1200;
const ASPECT = TARGET_W / TARGET_H; // 4:3

type Mode = "upload" | "url";

export function TransformationImageEditor({
  value,
  onChange,
}: {
  value: string;
  ...
  const uploadFn = useServerFn(uploadTransformationImageFromBase64);
    ...
    <div className="space-y-2">
      {value ? (
        <div className="flex items-start gap-3">
          <div className="relative aspect-[4/3] w-32 shrink-0 overflow-hidden rounded-[10px] border border-reps-border bg-reps-panel-soft">
            <img src={value} alt="Result photo" className="h-full w-full object-cover" />
          </div>
          <button
            type="button"
            onClick={() => onChange("")}
            className="inline-flex h-8 items-center gap-1 rounded-[10px] border border-reps-border bg-reps-panel-soft px-2 text-[12px] text-white/70 hover:bg-reps-panel"
          >
      ...
      {mode === "upload" && (
        <div className="rounded-[12px] border border-dashed border-reps-border bg-reps-panel-soft/60 p-4 text-center">
          <ImageIcon className="mx-auto h-5 w-5 text-white/40" />
          <p className="mt-1.5 text-[12px] text-white/70">Drop a JPG / PNG / WebP / HEIC</p>
          <button
            type="button"
            onClick={() => fileRef.current?.click()}
            className="mt-2 inline-flex h-8 items-center gap-1.5 rounded-[10px] bg-reps-orange px-3 text-[12px] font-semibold text-white hover:bg-reps-orange-hover"
          >
      ...
      {mode === "url" && (
        <div className="space-y-2 rounded-[12px] border border-reps-border bg-reps-panel-soft/60 p-3">
          <input
            ...
            placeholder="https://…"
            className="h-9 w-full rounded-[10px] border border-reps-border bg-reps-ink/60 px-3 text-[12px] text-white placeholder:text-white/40 focus:outline-none focus:ring-1 focus:ring-reps-orange"
          />
          <div className="flex items-center justify-between">
            <span className="text-[11px] text-white/45">We'll crop it to 1600 × 1200.</span>
            <button
              type="button"
              ...
              className="inline-flex h-8 items-center gap-1.5 rounded-[10px] bg-reps-orange px-3 text-[12px] font-semibold text-white hover:bg-reps-orange-hover disabled:opacity-60"
            >
...
function CropperModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4">
      <div className="w-full max-w-[560px] overflow-hidden rounded-[16px] border border-reps-border bg-reps-ink shadow-2xl">
        <div className="flex items-center justify-between border-b border-reps-border px-4 py-3">
          <div>
            <div className="text-[14px] font-semibold text-white">Crop result photo</div>
            <div className="text-[11px] text-white/55">Landscape 4:3 · saves as 1600 × 1200 JPEG</div>
          </div>
          <button
            type="button"
            onClick={onCancel}
