  ...
  .handler(async ({ data, context }): Promise<{ users: OnlineNowUser[]; timing: PanelTiming }> => {
    ...
    const { result, timing } = await measure("getOnlineNow", async () => {
      ...
      const [profilesRes, subsRes] = await Promise.all([
        userIds.length ? supabaseAdmin.from("profiles").select("id, full_name, full_name, avatar_url").in("id", userIds) : Promise.resolve({ data: [] as unknown[] }),
      ...
      for (const p of (profilesRes.data ?? []) as Array<{ id: string; full_name: string | null; full_name: string | null; avatar_url: string | null }>) {
        pMap.set(p.id, { name: p.full_name || p.full_name || p.id.slice(0, 8), email: null, avatar_url: p.avatar_url });
      }
  ...
  .handler(async ({ data, context }): Promise<{ pages: CurrentPageRow[]; timing: PanelTiming }> => {
    ...
    const { result, timing } = await measure("getCurrentPages", async () => {
      ...
      const profilesRes = allUserIds.length
        ? await supabaseAdmin.from("profiles").select("id, full_name, full_name, avatar_url").in("id", allUserIds)
        : { data: [] as unknown[] };
      const pMap = new Map<string, { name: string; avatar_url: string | null }>();
      for (const p of (profilesRes.data ?? []) as Array<{ id: string; full_name: string | null; full_name: string | null; avatar_url: string | null }>) {
        pMap.set(p.id, { name: p.full_name || p.full_name || p.id.slice(0, 8), avatar_url: p.avatar_url });
      }
  ...
  .handler(async ({ context }): Promise<{ rows: AttentionRow[]; timing: PanelTiming }> => {
    ...
    const { result, timing } = await measure("getNeedsAttention", async () => {
      ...
      const profilesRes = userIds.length
        ? await supabaseAdmin.from("profiles").select("id, full_name, full_name").in("id", userIds)
        : { data: [] as unknown[] };
      const pMap = new Map<string, string>();
      for (const p of (profilesRes.data ?? []) as Array<{ id: string; full_name: string | null; full_name: string | null }>) {
        pMap.set(p.id, p.full_name || p.full_name || p.id.slice(0, 8));
      }
      void iso7d;
  ...
  .handler(async ({ data, context }): Promise<EventDetail | null> => {
    ...
    if (userId) {
      const { data: p } = await supabaseAdmin
        .from("profiles")
        .select("id, full_name, full_name, avatar_url")
        .eq("id", userId)
        .maybeSingle();
      if (p) {
        const pp = p as { id: string; full_name: string | null; full_name: string | null; avatar_url: string | null };
        member = { user_id: pp.id, name: pp.full_name || pp.full_name || pp.id.slice(0, 8), email: null, avatar_url: pp.avatar_url };
      }
    }
    return {
      source: data.source,
