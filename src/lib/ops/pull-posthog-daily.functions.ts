...
function toTopN(rows: unknown[][] | undefined, limit = 10): Array<Record<string, unknown>> {
  if (!rows) return [];
  return rows.slice(0, limit).map((r) => ({ key: r[0], count: r[1] }));
}
...
export const pullPostHogDaily = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const target = data.date ?? new Date(Date.now() - 86_400_000).toISOString().slice(0, 10);
    ...
    try {
      ...
      const countries = await hogql(
        `SELECT properties.country_code, count() c FROM events ${where} GROUP BY properties.country_code ORDER BY c DESC LIMIT 20`,
        personalKey,
      );
