...
/** Mint a signed token: base64url(payload).base64url(hmac) where
 *  payload = `${slug}:${expEpochSec}`. Slug binds token to that pro. */
...
/** Verify token is well-formed, matches slug, not expired, HMAC matches. */
