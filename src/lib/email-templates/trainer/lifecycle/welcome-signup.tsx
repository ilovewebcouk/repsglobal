...
const WelcomeSignupEmail = ({ proName }: Props) => {
  const greeting = proName ? `Hi ${proName},` : "Hi,";
  return (
    <Html lang="en" dir="ltr">
      <Head />
      <Preview>Welcome to REPS — let's get you verified</Preview>
      <Body style={main}>
        <Container style={container}>
          <Section style={brandBar}><Text style={brandText}>{SITE_NAME}</Text></Section>
          <Heading style={h1}>Welcome to REPS</Heading>
          <Text style={text}>{greeting}</Text>
          <Text style={text}>
            Your account is live. To appear on the public register and start
            receiving enquiries, finish the three verification steps:
          </Text>
          <Text style={text}>
            1. ID check<br />
            2. Qualifications<br />
          ...
          <Text style={text}>
            Most pros complete it in under 10 minutes. Once approved, your
            profile is listed publicly with the verified badge.
          </Text>
...
const footer = { fontSize: "12px", color: "#8a8f99", margin: "0 0 6px" };
